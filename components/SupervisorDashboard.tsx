...
const EmptyState = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-[#1a1b3b]/40 border border-white/5 rounded-xl border-dashed col-span-full w-full">
...
const SupervisorDashboard: React.FC<SupervisorDashboardProps> = ({ user }) => {
  // View State: 'browse' | 'artist' | 'briefs' | 'briefApps'
  const [view, setView] = useState<'browse' | 'artist' | 'briefs' | 'briefApps'>('browse');
  ...
  const [briefApps, setBriefApps] = useState<any[]>([]); // Applications for a specific brief
  ...
  const loadBriefs = async () => {
    try {
      ...
      setBriefs(data || []);
      const counts = await getBriefApplicationCounts();
  ...
  const loadArtistData = async (artistId: string) => {
    try {
      ...
      const genres = validTracks.map(t => t.genre).filter(Boolean);
      ...
      if (genres.length > 0) {
          ...
          if (entries.length > 0) {
             topGenre = entries.reduce((a, b) => a[1] > b[1] ? a : b)[0];
          }
      }
  ...
  const handleBriefSelect = async (brief: SyncBrief) => {
    setSelectedBrief(brief);
    setView('briefApps');
    setLoading(true);
    try {
      const apps = await fetchBriefApplicationsWithDetails(brief.id);
      setBriefApps(apps || []);
    } catch (e) {
  ...
  const navigateToArtist = async (userId: string) => {
     setLoading(true);
     try {
  ...
  return (
    <div className="flex h-screen bg-[#31326f] overflow-hidden">
          ...
          <nav className="space-y-2">
            <button
               ...
               className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all
                  ${['browse', 'artist'].includes(view) ? 'bg-[#ccff00] text-black shadow-lg shadow-[#ccff00]/10' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
            >
               <Users className={`w-4 h-4 ${['browse', 'artist'].includes(view) ? 'text-black' : ''}`} />
               Talent
            </button>
            <button
               onClick={() => setView('briefs')}
               className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all
                  ${['briefs', 'briefApps'].includes(view) ? 'bg-[#ccff00] text-black shadow-lg shadow-[#ccff00]/10' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
            >
               <Briefcase className={`w-4 h-4 ${['briefs', 'briefApps'].includes(view) ? 'text-black' : ''}`} />
               Briefs
            </button>
          </nav>
        </div>
        ...
        <div className="mt-auto p-6 border-t border-white/10 bg-black/20">
           <div className="flex items-center gap-3 mb-4">
              ...
              <div className="overflow-hidden">
                 ...
                 <div className="text-[10px] text-white/50 uppercase tracking-wider">Supervisor</div>
              </div>
           </div>
           <button onClick={logoutUser} className="w-full flex items-center justify-center gap-2 py-2 text-xs font-bold text-red-400 hover:bg-red-500/10 rounded-xl transition-colors uppercase tracking-widest">
      ...
      <AnimatePresence>
         {isSidebarOpen && (
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} className="fixed inset-0 z-40 bg-[#0f1025] md:hidden pt-20 px-6">
               <nav className="space-y-4">
                  <button onClick={() => { setView('browse'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-4 py-4 rounded-xl text-sm font-bold uppercase tracking-widest transition-all ${['browse', 'artist'].includes(view) ? 'bg-[#ccff00] text-black' : 'text-white/60 bg-white/5'}`}>
                     <Users className="w-5 h-5" /> Talent
                  </button>
                  <button onClick={() => { setView('briefs'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-4 py-4 rounded-xl text-sm font-bold uppercase tracking-widest transition-all ${['briefs', 'briefApps'].includes(view) ? 'bg-[#ccff00] text-black' : 'text-white/60 bg-white/5'}`}>
                     <Briefcase className="w-5 h-5" /> Briefs
                  </button>
                  <button onClick={logoutUser} className="w-full flex items-center gap-4 px-4 py-4 rounded-xl text-sm font-bold uppercase tracking-widest text-red-400 bg-red-500/10 mt-8">
      ...
      <main className="flex-1 overflow-y-auto h-full relative pt-20 md:pt-0">
         <div className="p-6 md:p-12 pb-32 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 pb-6 border-b border-white/10">
               <div>
                  <h1 className="text-3xl md:text-5xl font-heading font-bold mb-2">Supervisor <span className="text-[#ccff00]">Console</span></h1>
                  <p className="text-white/50 font-mono text-sm">Welcome back, {user.displayName}</p>
               </div>
            </div>

            {/* Back Buttons */}
            <div className="mb-6">
               {view === 'artist' && <button onClick={() => { setView('browse'); setSelectedArtist(null); }} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-full text-white transition-colors text-sm"><ChevronLeft className="w-4 h-4" /> Back to Roster</button>}
               {view === 'briefApps' && <button onClick={() => { setView('briefs'); setSelectedBrief(null); }} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-full text-white transition-colors text-sm"><ChevronLeft className="w-4 h-4" /> Back to Briefs</button>}
            </div>

            {/* Main Content */}
            <AnimatePresence mode="wait">
               {/* VIEW 1: BROWSE ARTISTS */}
               {view === 'browse' && (
                  <motion.div key="browse" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                     <h2 className="text-xl font-bold font-heading mb-6 flex items-center gap-2"><Grid className="w-5 h-5 text-[#ccff00]" /> Browse Talent</h2>
                     {loading ? (
                        <div className="text-white/40 text-center py-20">Loading global roster...</div>
                     ) : !artists || artists.length === 0 ? (
                        <EmptyState icon={Ghost} title="No Artists Found" description="The roster is currently empty." />
                     ) : (
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-8">
                           {(artists || []).map((artist) => (
                              <div key={artist.id} onClick={() => { setSelectedArtist(artist); setView('artist'); }} className="group cursor-pointer">
                                 <div className="aspect-square rounded-full overflow-hidden bg-[#1a1b3b] mb-4 relative shadow-2xl border border-white/5 group-hover:border-[#ccff00]/50 transition-colors">
                                    <img src={artist.avatar_url || `https://ui-avatars.com/api/?name=${artist.full_name}&background=random`} alt={artist.full_name} className="w-full h-full object-cover" />
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Play className="w-12 h-12 text-white fill-white" /></div>
                                 </div>
                  ...
                  <motion.div key="artist" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                     <div className="flex flex-col md:flex-row gap-8 items-center md:items-end mb-8">
                        <div className="w-48 h-48 md:w-64 md:h-64 rounded-full overflow-hidden shadow-2xl border-4 border-[#ccff00]/20 shrink-0">
                           <img src={selectedArtist.avatar_url || `https://ui-avatars.com/api/?name=${selectedArtist.full_name}`} className="w-full h-full object-cover" alt={selectedArtist.full_name} />
                        </div>
                        <div className="text-center md:text-left w-full">
                           ...
                           <div className="flex flex-wrap gap-3 justify-center md:justify-start items-center">
                              ...
                              {selectedArtist.socials && <div className="flex gap-2 ml-2">{selectedArtist.socials.spotify && <a href={selectedArtist.socials.spotify} target="_blank" className="p-2 rounded-full bg-black/30 hover:bg-[#1DB954] text-white"><Music className="w-4 h-4" /></a>}{selectedArtist.socials.appleMusic && <a href={selectedArtist.socials.appleMusic} target="_blank" className="p-2 rounded-full bg-black/30 hover:bg-[#FA243C] text-white"><Disc className="w-4 h-4" /></a>}{selectedArtist.socials.instagram && <a href={selectedArtist.socials.instagram} target="_blank" className="p-2 rounded-full bg-black/30 hover:bg-[#E1306C] text-white"><LinkIcon className="w-4 h-4" /></a>}{selectedArtist.socials.website && <a href={selectedArtist.socials.website} target="_blank" className="p-2 rounded-full bg-black/30 hover:bg-blue-500 text-white"><Globe className="w-4 h-4" /></a>}</div>}
                           </div>
                        </div>
                     </div>
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
                        <div className="bg-[#1a1b3b]/40 border border-white/10 p-6 rounded-xl flex items-center gap-4"><div className="p-3 bg-[#ccff00]/10 text-[#ccff00] rounded-full"><Disc className="w-6 h-6" /></div><div><div className="text-2xl font-bold text-white font-heading">{artistStats.totalTracks}</div><div className="text-xs text-white/50 uppercase tracking-wider">Total Tracks</div></div></div>
                        <div className="bg-[#1a1b3b]/40 border border-white/10 p-6 rounded-xl flex items-center gap-4"><div className="p-3 bg-blue-500/10 text-blue-400 rounded-full"><Activity className="w-6 h-6" /></div><div><div className="text-2xl font-bold text-white font-heading">{artistStats.topGenre}</div><div className="text-xs text-white/50 uppercase tracking-wider">Top Genre</div></div></div>
                        <div className="bg-[#1a1b3b]/40 border border-white/10 p-6 rounded-xl flex items-center gap-4"><div className="p-3 bg-purple-500/10 text-purple-400 rounded-full"><Award className="w-6 h-6" /></div><div><div className="text-2xl font-bold text-white font-heading">{artistStats.totalApps}</div><div className="text-xs text-white/50 uppercase tracking-wider">Brief Applications</div></div></div>
                        <button onClick={() => setShowAppsModal(true)} className="bg-white/5 border border-white/10 hover:bg-white/10 hover:border-[#ccff00]/50 p-6 rounded-xl flex flex-col items-center justify-center gap-2 group transition-all h-full"><Briefcase className="w-8 h-8 text-white group-hover:text-[#ccff00]" /><span className="text-xs font-bold uppercase tracking-widest text-white group-hover:text-[#ccff00]">View App History</span></button>
                     </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {(briefs || []).map((brief) => (
                           <div key={brief.id} onClick={() => handleBriefSelect(brief)} className="bg-[#1a1b3b]/40 border border-white/10 p-6 rounded-xl hover:border-[#ccff00]/50 transition-colors cursor-pointer group">
                              <div className="flex justify-between items-start mb-4"><span className="px-2 py-1 bg-white/5 rounded text-[10px] uppercase tracking-wider font-bold text-white/50">{brief.client}</span><span className="px-2 py-1 bg-[#ccff00]/10 text-[#ccff00] rounded text-[10px] uppercase tracking-wider font-bold">{brief.deadline}</span></div>
                              <h3 className="text-lg font-bold text-white group-hover:text-[#ccff00] transition-colors mb-2">{brief.title}</h3>
                              <p className="text-sm text-white/50 line-clamp-2 mb-4">{brief.description}</p>
                              <div className="flex items-center text-[#ccff00] text-xs font-bold gap-2">View {briefCounts[brief.id] || 0} Applications <ArrowUpRight className="w-3 h-3" /></div>
                           </div>
                        ))}
                     </div>
                  </motion.div>
      ...
      <AnimatePresence>
        {showAppsModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setShowAppsModal(false)} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-[95%] md:w-full max-w-4xl bg-[#1a1b3b] border border-white/10 shadow-2xl rounded-xl overflow-hidden max-h-[90vh] flex flex-col">
               <div className="p-6 border-b border-white/10 bg-black/20 flex justify-between items-center shrink-0"><div><h3 className="font-heading font-bold text-xl text-white">Application History</h3><p className="text-xs text-white/50 uppercase tracking-widest">{selectedArtist?.full_name}</p></div><button onClick={() => setShowAppsModal(false)}><X className="w-5 h-5 text-white/50 hover:text-white" /></button></div>
               <div className="overflow-y-auto p-0">
                 <table className="w-full text-left min-w-[600px]">
                    <thead className="bg-white/5 text-xs font-bold uppercase tracking-widest text-white/40 sticky top-0 backdrop-blur-sm z-10"><tr><th className="p-4">Brief</th><th className="p-4">Track Submitted</th><th className="p-4">Submitted Date</th><th className="p-4 text-right">Status</th></tr></thead>
                    <tbody className="divide-y divide-white/5">{!artistApps || artistApps.length === 0 ? <tr><td colSpan={4} className="p-12"><EmptyState icon={Briefcase} title="No Applications" description="This artist hasn't applied to any briefs yet." /></td></tr> : (artistApps || []).map((app) => (<tr key={app.id} className="hover:bg-white/5 transition-colors"><td className="p-4"><div className="font-bold text-white text-sm">{getBriefTitle(app.briefId)}</div></td><td className="p-4 text-sm text-gray-300"><div className="flex items-center gap-2"><Music className="w-3 h-3 text-[#ccff00]" /> {getTrackTitle(app.trackId)}</div></td><td className="p-4 text-xs font-mono text-white/50">{app.submittedDate}</td><td className="p-4 text-right"><StatusBadge status={app.status} /></td></tr>))}</tbody>
                 </table>
               </div>
            </motion.div>
          </div>
